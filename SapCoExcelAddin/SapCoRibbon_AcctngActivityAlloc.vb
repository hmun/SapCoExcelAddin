Public Class SapCoRibbon_AcctngActivityAlloc
    ...
    Public Function getGenParameters(ByRef pPar As SAPCommon.TStr) As Integer
        ...
        Dim aName As String
        Dim i As Integer
        log.Debug("SapCoRibbon_AcctngActivityAlloc getGenParametrs - " & "reading Parameter")
    ...
    Public Sub Post(ByRef pSapCon As SapCon, Optional pCheck As Boolean = False)
        ...
        If Not getIntParameters(aIntPar) Then
            Exit Sub
        End If
        Dim aSAPAcctngActivityAlloc As New SAPAcctngActivityAlloc(pSapCon, aIntPar)
        ...
        Try
            log.Debug("SapCoRibbon_AcctngActivityAlloc.Post - " & "processing data - disabling events, screen update, cursor")
            ...
            Dim aTSAP_Data_Co As New TSAP_Data_Co(aPar, aIntPar, aSAPAcctngMeta, "postActivityAlloc")
            Do
                If Left(CStr(aCoWs.Cells(i, aMsgClmnNr).Value), Len(aOKMsg)) <> aOKMsg Then
                    ...
                    If String.IsNullOrEmpty(CStr(aCoWs.Cells(i + 1, aCoClmnNr).value)) Or aPost.ToUpper = "X" Then
                        If aTSAP_Data_Co.fillHeader(aItems) And aTSAP_Data_Co.fillData(aItems) Then
                            log.Debug("SapCoRibbon_AcctngActivityAlloc.Post - " & "calling aSAPAcctngActivityAlloc.Post")
                            Globals.ThisAddIn.Application.StatusBar = "Calling SAP-BAPI at line " & i
                            aRetStr = aSAPAcctngActivityAlloc.Post(aTSAP_Data_Co, pOKMsg:=aOKMsg, pCheck:=pCheck)
                            log.Debug("SapCoRibbon_AcctngActivityAlloc.Post - " & "aSAPAcctngActivityAlloc.Post returned, aRetStr=" & aRetStr)
                            For Each aKey In aItems.aTDataDic.Keys
                                aCoWs.Cells(CInt(aKey), aMsgClmnNr) = CStr(aRetStr)
                            Next
                        End If
                        aItems = New TData(aIntPar)
                        aTSAP_Data_Co = New TSAP_Data_Co(aPar, aIntPar, aSAPAcctngMeta, "postActivityAlloc")
                    End If
                End If
                i += 1
            Loop While Not String.IsNullOrEmpty(CStr(aCoWs.Cells(i, aCoClmnNr).value))
            log.Debug("SapCoRibbon_AcctngActivityAlloc.Post - " & "all data processed - enabling events, screen update, cursor")
        ...
        Catch ex As System.Exception
            ...
            Globals.ThisAddIn.Application.ScreenUpdating = True
            Globals.ThisAddIn.Application.Cursor = Microsoft.Office.Interop.Excel.XlMousePointer.xlDefault
            MsgBox("SapCoRibbon_AcctngActivityAlloc.Post failed! " & ex.Message, MsgBoxStyle.OkOnly Or MsgBoxStyle.Critical, "SAP Co AddIn")
            log.Error("SapCoRibbon_AcctngActivityAlloc.Post - " & "Exception=" & ex.ToString)
            Exit Sub
        End Try
    End Sub
