Public Class SapCoRibbon_AcctngRepstPrimCosts
    ...
    Public Function getGenParameters(ByRef pPar As SAPCommon.TStr) As Integer
        ...
        Dim aName As String
        Dim i As Integer
        log.Debug("SapCoRibbon_AcctngRepstPrimCosts getGenParametrs - " & "reading Parameter")
    ...
    Public Sub Post(ByRef pSapCon As SapCon, Optional pCheck As Boolean = False)
        ...
        If Not getIntParameters(aIntPar) Then
            Exit Sub
        End If
        Dim aSAPAcctngRepstPrimCosts As New SAPAcctngRepstPrimCosts(pSapCon, aIntPar)
        ...
        Try
            log.Debug("SapCoRibbon_AcctngRepstPrimCosts.Post - " & "processing data - disabling events, screen update, cursor")
            ...
            Dim aTSAP_Data_Co As New TSAP_Data_Co(aPar, aIntPar, aSAPAcctngMeta, "postRepstPrimCosts")
            Do
                If Left(CStr(aCoWs.Cells(i, aMsgClmnNr).Value), Len(aOKMsg)) <> aOKMsg Then
                    ...
                    If String.IsNullOrEmpty(CStr(aCoWs.Cells(i + 1, aCoClmnNr).value)) Or aPost.ToUpper = "X" Then
                        If aTSAP_Data_Co.fillHeader(aItems) And aTSAP_Data_Co.fillData(aItems) Then
                            log.Debug("SapCoRibbon_AcctngRepstPrimCosts.Post - " & "calling aSAPAcctngRepstPrimCosts.Post")
                            Globals.ThisAddIn.Application.StatusBar = "Calling SAP-BAPI at line " & i
                            aRetStr = aSAPAcctngRepstPrimCosts.Post(aTSAP_Data_Co, pOKMsg:=aOKMsg, pCheck:=pCheck)
                            log.Debug("SapCoRibbon_AcctngRepstPrimCosts.Post - " & "aSAPAcctngRepstPrimCosts.Post returned, aRetStr=" & aRetStr)
                            For Each aKey In aItems.aTDataDic.Keys
                                aCoWs.Cells(CInt(aKey), aMsgClmnNr) = CStr(aRetStr)
                            Next
                        End If
                        aItems = New TData(aIntPar)
                        aTSAP_Data_Co = New TSAP_Data_Co(aPar, aIntPar, aSAPAcctngMeta, "postRepstPrimCosts")
                    End If
                End If
                i += 1
            Loop While Not String.IsNullOrEmpty(CStr(aCoWs.Cells(i, aCoClmnNr).value))
            log.Debug("SapCoRibbon_AcctngRepstPrimCosts.Post - " & "all data processed - enabling events, screen update, cursor")
        ...
        Catch ex As System.Exception
            ...
            Globals.ThisAddIn.Application.ScreenUpdating = True
            Globals.ThisAddIn.Application.Cursor = Microsoft.Office.Interop.Excel.XlMousePointer.xlDefault
            MsgBox("SapCoRibbon_AcctngRepstPrimCosts.Post failed! " & ex.Message, MsgBoxStyle.OkOnly Or MsgBoxStyle.Critical, "SAP Co AddIn")
            log.Error("SapCoRibbon_AcctngRepstPrimCosts.Post - " & "Exception=" & ex.ToString)
            Exit Sub
        End Try
    End Sub
