...
Public Class TSAP_Data_CoPa
    ...
    Public Sub New(ByRef pPar As SAPCommon.TStr, ByRef pIntPar As SAPCommon.TStr, ByRef pSAPAcctngMeta As SAPAcctngMeta, pFunction As String)
        aPar = pPar
        aIntPar = pIntPar
        aFunction = pFunction
        ...
        ' get Metadata
        If pFunction = "postActivityAlloc" Then
            pSAPAcctngMeta.getMeta_AcctngActivityAllocPost(aParamDic, aStrucDic)
        ElseIf pFunction = "postManCostAlloc" Then
            pSAPAcctngMeta.getMeta_AcctngManCostAllocPost(aParamDic, aStrucDic)
        ElseIf pFunction = "postRepstPrimCosts" Then
            pSAPAcctngMeta.getMeta_AcctngRepstPrimCostsPost(aParamDic, aStrucDic)
        ElseIf pFunction = "postStatKeyFigures" Then
            pSAPAcctngMeta.getMeta_AcctngStatKeyFiguresPost(aParamDic, aStrucDic)
        End If
    End Sub
        ...
        For Each aKvB In pData.aTDataDic
            ...
            For Each aTStrRec In aTDataRec.aTDataRecCol
                If valid_Table_Field(aTStrRec) Then
                    ...
                    If aTStrRec.Strucname = "INPUTDATA" Then
                        aNewTStrRec = New SAPCommon.TStrRec("INPUTDATA", "RECORD_ID", CStr(aCnt))
                        aDataDic.addValue(CStr(aCnt), aNewTStrRec)
                        If aTStrRec.Currency <> "" Then
                            aNewTStrRec = New SAPCommon.TStrRec("INPUTDATA", "CURRENCY", aTStrRec.Currency)
                            aDataDic.addValue(CStr(aCnt), aNewTStrRec)
                        End If
                        aNewTStrRec = New SAPCommon.TStrRec("FIELDLIST", "FIELDNAME", aTStrRec.Value)
                        aDataDic.addValue(CStr(aCnt), aNewTStrRec)
                    End If
                End If
            Next
